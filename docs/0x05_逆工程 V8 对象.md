...
了解了这些内容，我们对于一个给定的地址 `p`，判断其为 `Heap Object` 后可以首先通过 `Heap Object` 的对象布局包含的 `kMapOffset` 来计算其指向的 `Meta Map` 的真实地址；

按照引擎的定义 `Meta Map` 的起始伪指针指向一个 `Meta Map root` (作为 GC root 方便 GC)，换言之所有的 JS 对象指向的 `Meta Map` 都指向了同一个 `Meta Map root`。
