...
这里的逆工程起始就是根据 V8 设计的对 JS 对象存储结构的描述，开发者可以实现在进程运行内存空间中或者进程崩溃后的 Core 文件还原的内存空间中来反推出当前 JS 代码运行状态和 JS 对象在堆空间的分配状况的一种技术。

这种技术可以帮助开发者分析运行中的 JS 程序，或者根据 JS 程序崩溃生成的 Core 文件来进行事后分析崩溃原因，本文主要从 V8 对象布局的角度来和大家探讨学习下逆工程背后的一些知识。
...
起始按照引擎的定义， `Meta Map` 的起始伪指针指向一个 `Meta Map root` (作为 GC root 方便 GC)，换言之所有的 JS 对象指向的 `Meta Map` 都指向了同一个 `Meta Map root`:
